...
def main():
    ...
    es = [0.01, 0.02, 0.03, 0.04, 0.06, 0.08, 0.1, 
         0.12, 0.14, 0.16, 0.18, 0.2,
         0.22, 0.24, 0.26, 0.28, 0.3,
         0.32, 0.34, 0.36, 0.38, 0.4]

    for erasure_prob in es:
        repeats = 500
        gaps = []
        gsuccess = 0
        ...
        for t in range(repeats):
            ...
            # Extract only the non-zero rows of sHx into matrix A
            nonzero_row_mask = np.any(sHx != 0, axis=1)
            nonzero_row_indices = np.nonzero(nonzero_row_mask)[0]
            A = sHx[nonzero_row_indices, :].copy()

            # Remove duplicate rows from A, keeping the first occurrence (preserve original row order)
            if A.size:
                seen = set()
                unique_idx = []
            ...
            #print(f"Extracted {A.shape[0]} non-zero rows into A (shape {A.shape}) from sHx (shape {sHx.shape}).")

            if show_sampled_matrix:
                # (Optional) visualize the reduced matrix A
                plt.figure(figsize=(8, 6))
                plt.imshow(A, cmap='Greys', interpolation='nearest', aspect='auto')
                plt.xlabel('column')
                plt.ylabel('row (non-zero only)')
                plt.title('A: non-zero rows of sampled sHx')
                plt.colorbar(label='value')
                plt.tight_layout()
                plt.savefig('A_nonzero_rows.png', dpi=200)
                plt.show()


            # --- Eigenvalue gap analysis via singular values of A ---
            # Since A is non-square, we use the singular values of A (sigma_i),
            # which are the square roots of the eigenvalues of the symmetric PSD matrix A @ A^T.
            # The "eigenvalue gap" is measured on A @ A^T: gap = lambda_1 - lambda_2
            # where lambda_i = sigma_i^2, sorted in descending order.
            # A large gap relative to lambda_1 (the spectral gap ratio) indicates a sparse/low-rank structure.
            A_float = A.astype(float)
            singular_values = np.linalg.svd(A_float, compute_uv=False)  # sorted descending
            eigenvalues = singular_values ** 2  # eigenvalues of A @ A^T

            #print(f"\n--- Eigenvalue gap analysis of A @ A^T ---")
            #print(f"  Top 10 eigenvalues: {eigenvalues[:10]}")
            if len(eigenvalues) >= 2:
                gap = eigenvalues[0] - eigenvalues[1]
                spectral_gap_ratio = gap / eigenvalues[0] if eigenvalues[0] > 0 else 0.0
                #print(f"  lambda_1 (largest):  {eigenvalues[0]:.6f}")
                #print(f"  lambda_2 (2nd):      {eigenvalues[1]:.6f}")
                #print(f"  Eigenvalue gap:      {gap:.6f}")
                #print(f"  Spectral gap ratio (gap / lambda_1): {spectral_gap_ratio:.6f}")
                gaps.append((gap, spectral_gap_ratio))
            else:
                print("  Not enough eigenvalues to compute a gap.")
            ...
            #print(f"  Effective rank of A: {effective_rank:.4f}  (out of {A.shape[0]} rows x {A.shape[1]} cols)")

            if show_eigenvalue_gap_analysis:
                # Plot the eigenvalue spectrum
                plt.figure(figsize=(8, 4))
                plt.plot(eigenvalues, 'o-', markersize=4)
                plt.xlabel('Index')
                plt.ylabel('Eigenvalue of A @ Aᵀ')
                plt.title('Eigenvalue spectrum of A @ Aᵀ')
                plt.yscale('log')
                plt.tight_layout()
                plt.savefig('A_eigenvalue_spectrum.png', dpi=200)
                plt.show()

            sn = len(sampled_col_indices)
            rank = gf2_rank(A)
            if rank >= sn:
                gsuccess += 1

        print(erasure_prob, 
              sum([g[0] for g in gaps])/len(gaps), 
